...
# justc-nixos/ directory, or next to justc-nixos-v9-reviewed.zip. A renamed
# justc-nixos.zip is accepted only when it contains the v9 review marker. The
...
if ! sudo test -e "$TOP_MOUNT/@/etc/NIXOS" &&
   ! sudo test -L "$TOP_MOUNT/@/etc/NIXOS"; then
  die "The existing @ does not identify itself as NixOS; refusing to replace it."
fi
...
elif [[ "$PROFILE_TARGET" =~ ^/nix/store/[a-z0-9]{32}-[^/]+$ ]]; then
...
[[ "$OLD_SYSTEM_LOGICAL" =~ ^/nix/store/[a-z0-9]{32}-[^/]+$ ]] ||
  die "The previous generation does not point to one exact Nix store path."
OLD_SYSTEM_PHYSICAL="$TOP_MOUNT/@nix${OLD_SYSTEM_LOGICAL#/nix}"
sudo test -d "$OLD_SYSTEM_PHYSICAL" ||
  die "The previous system closure is missing: $OLD_SYSTEM_PHYSICAL"
